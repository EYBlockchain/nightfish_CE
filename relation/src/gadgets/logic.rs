...
impl<F: PrimeField> PlonkCircuit<F> {
    ...
    pub fn is_equal(&mut self, a: Variable, b: Variable) -> Result<BoolVar, CircuitError> {
        ...
        // y is the bit indicating if a == b
        // c_inv is the inverse of c = a - b when a != b
        let c_val = self.witness(a)? - self.witness(b)?;
        let (y, c_inv) = if c_val.is_zero() {
            (F::one(), F::zero())
        } else {
            (
                F::zero(),
                c_val.inverse().ok_or_else(|| {
                    CircuitError::FieldAlgebraError("Unable to find inverse".to_string())
                })?,
            )
        };
        let y = self.create_boolean_variable_unchecked(y)?;
        let c_inv = self.create_variable(c_inv)?;

        // constraint 1: 1 - (a-b) * c^(-1) = y, i.e., (a-b) * c^(-1) - 1 = -y
        self.quad_poly_gate(
            &[a, c_inv, b, c_inv, y.into()],
            &[F::zero(), F::zero(), F::zero(), F::zero()],
            &[F::one(), -F::one()],
            -F::one(),
            -F::one(),
        )?;
        // constraint 2: multiplication (a-b) * y = 0
        self.mul_add_gate(
            &[a, y.into(), b, y.into(), self.zero()],
            &[F::one(), -F::one()],
        )?;
        Ok(y)
    }
    ...
    pub fn logic_neg(&mut self, a: BoolVar) -> Result<BoolVar, CircuitError> {
        let res_var = self.sub(self.one(), a.into())?;
        Ok(BoolVar(res_var))
    }
