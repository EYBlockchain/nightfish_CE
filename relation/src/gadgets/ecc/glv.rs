    ...
    F: PrimeField,
{
        ...
        P: HasTEForm<BaseField = F>,
    {
        ...
        let endo_base_var = endomorphism_circuit::<_>(self, base)?;
...
fn endomorphism<F>(base: &Point<F>) -> Point<F>
where
    F: PrimeField,
{
    let x = base.get_x();
    ...
    let te_x = (f_y * h_y) / (h_y * xy);
    let te_y = (g_y * xy) / (h_y * xy);

    Point::TE(te_x, te_y)
}

/// The circuit for computing the point endomorphism.
fn endomorphism_circuit<F>(
    circuit: &mut PlonkCircuit<F>,
...
where
    F: PrimeField,
{
    let point_wrapper = circuit.point_witness(point_var)?;
    let base = match point_wrapper {
        Point::TE(..) => Ok(point_wrapper),
        _ => Err(CircuitError::ParameterError("Expected TEPoint".to_string())),
    }?;
    let endo_point = endomorphism::<_>(&base);
    ...
    S: PrimeField,
{
    ...
    #[cfg(test)]
    {
        ...
        let k2_bits = get_bits(&k2.into_bigint().to_bits_le());

        assert!(k1_bits < 128, "k1 bits {}", k1_bits);
        assert!(k2_bits < 128, "k2 bits {}", k2_bits);
...
mod tests {
    ...
    use ark_ec::{short_weierstrass::Affine as SWAffine, CurveConfig};
    use ark_ff::{BigInteger, MontFp, One, PrimeField, UniformRand};
    use jf_utils::{field_switching, fr_to_fq, test_rng};
    use nf_curves::ed_on_bls_12_381_bandersnatch::{EdwardsAffine, EdwardsConfig, Fq, Fr};
        ...
        P: HasTEForm<BaseField = F>,
    {
        ...
        for _ in 0..100 {
...

            {
                ...
                let base_var = circuit.create_point_variable(&Point::from(base))?;
                base = (base * s).into();
                let result = circuit.glv_mul_old::<P>(s_var, &base_var)?;
                assert_eq!(
                    TEPoint::from(base),
                    TEPoint(
                        circuit.witness(result.get_x())?,
    ...
    fn test_endomorphism() {
        ...
        let t = endomorphism::<_>(&base_point);
        ...
        let endo_var = endomorphism_circuit::<_>(&mut circuit, &point_var).unwrap();
        let endo_point_rec = Point::TE(
            circuit.witness(endo_var.get_x()).unwrap(),
            circuit.witness(endo_var.get_y()).unwrap(),
        );
