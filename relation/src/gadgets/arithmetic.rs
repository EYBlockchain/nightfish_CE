...
impl<F: PrimeField> PlonkCircuit<F> {
    ...
    /// Returns a variable to the fifth power pf the input variable.
    pub fn power_5_gen(&mut self, x: Variable) -> Result<Variable, CircuitError> {
        self.check_var_bound(x)?;

        let val = self.witness(x)?;
        let val_to_5 = val.pow([5]);
        let var = self.create_variable(val_to_5)?;
        let wire_vars = &[x, 0, 0, 0, var];
        self.insert_gate(wire_vars, Box::new(FifthRootGate))?;
        Ok(var)
    }
