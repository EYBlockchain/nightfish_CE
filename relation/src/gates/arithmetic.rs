    ...
    F: Field,
{
    ...
    /// Selector 1 for X coordinate in SW point addition.
    fn q_x(&self) -> F {
        F::one()
    }
    /// Selector 2 for X coordinate in SW point addition.
    fn q_x2(&self) -> F {
        F::one()
    }
    /// Selector 1 for Y coordinate in SW point addition.
    fn q_y(&self) -> F {
        F::one()
    }
    /// Selector 2 for Y coordinate in SW point addition.
    fn q_y2(&self) -> F {
        F::one()
    }
...
impl<F> LinCombGate<F>
where
    F: Field,
{
    /// Create new `LinCombGate` for use outside of crate.
    pub fn new(coeffs: [F; GATE_WIDTH], constant: F) -> LinCombGate<F> {
        LinCombGate { coeffs, constant }
    }
}
