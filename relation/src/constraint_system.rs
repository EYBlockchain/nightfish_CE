...
use crate::{
    ...
    errors::CircuitError::{self, *},
    gates::*,
    RecursionHasher,
};
use ark_ff::PrimeField;
use ark_poly::{
    domain::Radix2EvaluationDomain, univariate::DensePolynomial, DenseMultilinearExtension,
    DenseUVPolynomial, EvaluationDomain, MultilinearExtension,
};
use ark_std::{
    boxed::Box, cfg_chunks, cfg_into_iter, cfg_iter, cmp::max, format, string::ToString, sync::Arc,
    vec, vec::Vec,
};
...
pub trait Circuit<F: PrimeField> {
    ...
    /// Common gates that should be implemented in any constraint systems.
    ///
    /// Constrain a the first variable to be zero and also enforce that all the selectors have at least one non-zero value.
    /// Return error if `var` is an invalid variable.
    fn setup_gate(&mut self, var: Variable) -> Result<(), CircuitError>;
...
pub trait Arithmetization<F: PrimeField>: Circuit<F> {
    ...
    /// Returns the number of variables each MLE requires
    fn num_mle_vars(&self) -> Result<usize, CircuitError>;

    /// Compute and return selector polynomials.
    /// Return an error if the circuit has not been finalized yet.
    fn compute_selector_mles(&self)
        -> Result<Vec<Arc<DenseMultilinearExtension<F>>>, CircuitError>;

    /// Compute and return extended permutation polynomials.
    /// Return an error if the circuit has not been finalized yet.
    fn compute_extended_permutation_mles(
        &self,
    ) -> Result<Vec<Arc<DenseMultilinearExtension<F>>>, CircuitError>;

    /// Compute and return the extended identity permutation polynomials.
    /// Return an error if the circuit has not been finalized yet.
    fn compute_extended_identity_mles(
        &self,
    ) -> Result<Vec<Arc<DenseMultilinearExtension<F>>>, CircuitError>;

    /// Compute and return the product polynomials for permutation arguments.
    /// Return an error if the circuit has not been finalized yet.
    fn compute_prod_permutation_mles(
        &self,
        wire_mles: &[Arc<DenseMultilinearExtension<F>>],
        perm_mles: &[Arc<DenseMultilinearExtension<F>>],
        gamma: &F,
    ) -> Result<Vec<[Vec<F>; 2]>, CircuitError>;

    /// Compute and return the list of wiring witness polynomials.
    /// Return an error if the circuit has not been finalized yet.
    fn compute_wire_mles(&self) -> Result<Vec<Arc<DenseMultilinearExtension<F>>>, CircuitError>;

    /// Compute and return the public input polynomial.
    /// Return an error if the circuit has not been finalized yet.
    /// The IO gates of the circuit are guaranteed to be in the front.
    fn compute_pub_input_mle(&self) -> Result<Arc<DenseMultilinearExtension<F>>, CircuitError>;
    ...
    /// elements. Return an error if the circuit does not support lookup or
    /// has not been finalized yet.
    fn compute_range_table_mle(&self) -> Result<Arc<DenseMultilinearExtension<F>>, CircuitError> {
        Err(CircuitError::LookupUnsupported)
    }
    ...
    /// elements. Return an error if the circuit does not support lookup or
    /// has not been finalized yet.
    fn compute_key_table_mle(&self) -> Result<Arc<DenseMultilinearExtension<F>>, CircuitError> {
        Err(CircuitError::LookupUnsupported)
    }
    ...
    /// sepration ids. Return an error if the circuit does not support
    /// lookup or has not been finalized.
    fn compute_table_dom_sep_mle(&self) -> Result<Arc<DenseMultilinearExtension<F>>, CircuitError> {
        Err(CircuitError::LookupUnsupported)
    }
    ...
    /// sepration selectors for the lookup gates. Return an error if the
    /// circuit does not support lookup or has not been finalized.
    fn compute_q_dom_sep_mle(&self) -> Result<Arc<DenseMultilinearExtension<F>>, CircuitError> {
        Err(CircuitError::LookupUnsupported)
    }

    /// Compute and return the polynomial that interpolates the lookup
    /// selectors for the lookup gates. Return an error if the
    /// circuit does not support lookup or has not been finalized.
    fn compute_q_lookup_mle(&self) -> Result<Arc<DenseMultilinearExtension<F>>, CircuitError> {
        Err(CircuitError::LookupUnsupported)
    }

    /// Compute and return the combined lookup table vector given random
    /// challenge `tau`.
    fn compute_merged_lookup_table_mle(&self, _tau: F) -> Result<Vec<F>, CircuitError> {
        Err(CircuitError::LookupUnsupported)
    }
    ...
    /// error if the circuit does not support lookup or has not been
    /// finalized yet.
    fn compute_lookup_sorted_vec_mles(
        &self,
        _tau: F,
        _lookup_table: &[F],
    ) -> Result<Arc<DenseMultilinearExtension<F>>, CircuitError> {
        Err(CircuitError::LookupUnsupported)
    }

    /// Returns a boolean value if this circuit is suitable for recursive proving.
...
pub struct PlonkParams {
    ...
    pub plonk_type: PlonkType,

    /// The bit length of a range-check. None for TurboPlonk.
    pub range_bit_len: Option<usize>,
}
    ...
    F: PrimeField,
{
    ...
    witness: Vec<F>,
    /// The public input indices in the witness vector.
    pub pub_input_indices: Vec<usize>,
    ...
    pub wire_permutation: Vec<(WireId, GateId)>,
    ...
    /// Flag for if this circuit is suitable for recursive proving.
    recursion_flag: bool,
}

impl<F: PrimeField> Default for PlonkCircuit<F> {
    fn default() -> Self {
...
impl<F: PrimeField> PlonkCircuit<F> {
    ...
    fn new(plonk_params: PlonkParams) -> Self {
        ...
        circuit.setup_gate(0).unwrap(); // safe unwrap
    ...
    pub fn witness_vals(&self) -> Vec<F> {
        self.witness.clone()
    }

    /// Getter for wire variables
    pub fn wire_variables(&self) -> &[Vec<Variable>; GATE_WIDTH + 2] {
        &self.wire_variables
    }

    /// Getter for `self.pub_input_gate_ids`
    pub fn pub_input_gate_ids(&self) -> &[GateId] {
        &self.pub_input_gate_ids
    }

    /// Getter for `self.extended_id_permutation`
    pub fn extended_id_permutation(&self) -> &[F] {
    ...
    /// Constrains the circuit such that 'res_var' represents the appropriate
    /// linear combination of the values of the input 'Variable's.
    pub fn lin_comb_gate(
        &mut self,
        coeffs: &[F],
        constant: &F,
        vars: &[Variable],
        ...
        // If there are 4 or fewer 'Variables's we can just construct the appropriate gate.
        // Else we recurse.
        if coeffs.len() != vars.len() {
            return Err(CircuitError::ParameterError(format!(
                "coeffs and vars must be of same length. coeffs: {}, vars: {}",
                coeffs.len(),
                vars.len()
            )));
        }
        if coeffs.len() <= 4 {
            let mut gate_coeffs = [F::zero(); 4];
            let mut wire_vars = [Variable::default(); 5];
            for (i, (coeff, var)) in coeffs.iter().zip(vars.iter()).enumerate() {
                gate_coeffs[i] = *coeff;
                wire_vars[i] = *var;
            }
            wire_vars[4] = *res_var;
            self.insert_gate(
                &wire_vars,
                Box::new(LinCombGate::<F>::new(gate_coeffs, *constant)),
            )?;
        } else {
            let mut gate_coeffs = [F::zero(); 4];
            let mut wire_vars = [Variable::default(); 5];
            let mut val = *constant;
            for (i, (coeff, var)) in coeffs.iter().zip(vars.iter()).enumerate().take(4) {
                gate_coeffs[i] = *coeff;
                wire_vars[i] = *var;
                val += self.witness(*var)? * coeff;
            }
            wire_vars[4] = self.create_variable(val)?;
            self.insert_gate(
                &wire_vars,
                Box::new(LinCombGate::<F>::new(gate_coeffs, *constant)),
            )?;
            let mut new_vars = vars[4..].to_vec();
            let mut new_coeffs = coeffs[4..].to_vec();
            new_vars.push(wire_vars[4]);
            new_coeffs.push(F::one());
            self.lin_comb_gate(&new_coeffs, &F::zero(), &new_vars, res_var)?;
        }
        Ok(())
    }

    /// Returns the 'Variable' representing the appropriate
    /// linear combination of the values of the input 'Variable's.
    pub fn lin_comb(
        &mut self,
        coeffs: &[F],
        ...
        let res_var = self.create_variable(val)?;
        self.lin_comb_gate(coeffs, constant, vars, &res_var)?;
        Ok(res_var)
    }
}
...
impl<F: PrimeField> Circuit<F> for PlonkCircuit<F> {
    ...
    fn check_circuit_satisfiability(&self, pub_input: &[F]) -> Result<(), CircuitError> {
        ...
        for (i, &witness_index) in self.pub_input_indices.iter().enumerate() {
            if self.witness(witness_index)? != pub_input[i] {
                return Err(ParameterError(format!("Public input {} does not matched the supplied public input, supplied pi: {}, actual pi: {}",i, pub_input[i], self.witness(witness_index)?)));
            }
        }

        // Check rest of the gates
        cfg_into_iter!(0..self.num_gates()).try_for_each(|gate_id| {
    ...
    fn set_variable_public(&mut self, var: Variable) -> Result<(), CircuitError> {
        self.check_finalize_flag(false)?;
        self.pub_input_indices.push(var);
    ...
    fn setup_gate(&mut self, var: Variable) -> Result<(), CircuitError> {
        ...
        let wire_vars = &[0, 0, 0, 0, var];
        self.insert_gate(wire_vars, Box::new(SetupGate))?;
        Ok(())
    }
...
impl<F: PrimeField> PlonkCircuit<F> {
    ...
    /// Returns true if a circuit is ready to be preprocessed.
    ...
    fn rearrange_gates(&mut self, size: usize) -> Result<(), CircuitError> {
    ...
    // arithmetization.
    fn pad(&mut self, size: usize) -> Result<(), CircuitError> {
        self.check_finalize_flag(true)?;

        for _ in self.num_gates()..size {
            self.gates.push(Box::new(PaddingGate));
        }
        for wire_id in 0..self.num_wire_types() {
            self.wire_variables[wire_id].resize(size, self.zero());
        }
        Ok(())
    }
    ...
    fn check_gate(&self, gate_id: Variable, pub_input: &F) -> Result<(), CircuitError> {
        ...
        let q_x = self.gates[gate_id].q_x();
        let q_x2 = self.gates[gate_id].q_x2();
        let q_y = self.gates[gate_id].q_y();
        let q_y2 = self.gates[gate_id].q_y2();
        ...
        let expected_gate_output = *pub_input
            ...
            + q_x
                * (w_vals[0] * w_vals[3] * w_vals[2] * w_vals[3]
                    + w_vals[1] * w_vals[2] * w_vals[2] * w_vals[3])
            + q_x2
                * (w_vals[0] * w_vals[2]
                    + w_vals[1] * w_vals[3]
                    + F::from(2u8) * w_vals[0] * w_vals[3]
                    + F::from(2u8) * w_vals[1] * w_vals[2])
            + q_y * (w_vals[2] * w_vals[2] * w_vals[3] * w_vals[3])
            + q_y2 * (w_vals[0] * w_vals[0] * w_vals[1] + w_vals[0] * w_vals[1] * w_vals[1])
            + q_c;
    ...
    #[inline]
    fn compute_wire_permutation(&mut self, size: usize) {
        ...
        self.wire_permutation = vec![(0usize, 0usize); self.num_wire_types * size];
        for wires_vec in variable_wires_map.iter_mut() {
            ...
            if !wires_vec.is_empty() {
                ...
                for window in wires_vec.windows(2) {
                    self.wire_permutation[window[0].0 * size + window[0].1] = window[1];
                }
    ...
    #[inline]
    pub fn check_finalize_flag(&self, expect_finalized: bool) -> Result<(), CircuitError> {
    ...
    // getter for SW ecc x selector
    #[inline]
    fn q_x(&self) -> Vec<F> {
        self.gates.iter().map(|g| g.q_x()).collect()
    }
    // getter for SW ecc x selector 2
    #[inline]
    fn q_x2(&self) -> Vec<F> {
        self.gates.iter().map(|g| g.q_x2()).collect()
    }
    // getter for SW ecc y selector
    #[inline]
    fn q_y(&self) -> Vec<F> {
        self.gates.iter().map(|g| g.q_y()).collect()
    }
    // getter for SW ecc y selector 2
    #[inline]
    fn q_y2(&self) -> Vec<F> {
        self.gates.iter().map(|g| g.q_y2()).collect()
    }
    ...
    #[inline]
    pub fn table_dom_sep_vec(&self) -> Vec<F> {
    ...
    pub fn all_selectors(&self) -> Vec<Vec<F>> {
        ...
        selectors.push(self.q_x());
        selectors.push(self.q_x2());
        selectors.push(self.q_y());
        selectors.push(self.q_y2());
        if self.support_lookup() {
...
impl<F: PrimeField> PlonkCircuit<F> {
    ...
    /// wires. Refer to Sec 5.2 and Sec 8.1 of https://eprint.iacr.org/2019/953.pdf for more details.
    #[inline]
    pub fn compute_extended_mle_id_permutation(&mut self, size: usize) {
        assert!(self.is_finalized());

        // Compute the extended identity permutation
        // id[i*n+j] = i * n + j
        self.extended_id_permutation = cfg_into_iter!(0..self.num_wire_types() * size)
            .map(|i| F::from(i as u32))
            .collect();
    }

    #[inline]
    /// Compute the extended permutation for MLE arithmetisation.
    /// This is actually the identity permutation - permutation because
    /// we prove the permutation via GKR and this saves on calculating identity evaluations.
    pub fn compute_extended_mle_permutation(&self, size: usize) -> Result<Vec<F>, CircuitError> {
        assert!(self.is_finalized());
        ...
        let extended_perm: Vec<F> = self
            ...
            .iter()
            .enumerate()
            .map(|(i, &(wire_id, gate_id))| {
                // if permutation value undefined, return 0
                if wire_id >= self.num_wire_types {
                    F::zero()
                } else {
                    self.extended_id_permutation[i]
                        - self.extended_id_permutation[wire_id * size + gate_id]
                }
            })
            .collect();
        if extended_perm.len() != self.num_wire_types * size {
            return Err(ParameterError(
                "Length of the extended permutation vector should be number of gate \
                         (including padded dummy gates) * number of wire types"
                    .to_string(),
...
impl<F: PrimeField> PlonkCircuit<F> {
    ...
    pub fn finalize_for_arithmetization(&mut self) -> Result<(), CircuitError> {
        ...
        // here we add lookup variables to ensure no selectors are zero in the circuit
        if self.support_lookup() {
            self.add_range_check_variable(1).unwrap(); // safe unwrap
            let lookup_vars = &[(0, 1, 1), (1, 1, 1)];
            let table_vars = &[(1, 1), (1, 1), (1, 1)];
            self.create_table_and_lookup_variables(lookup_vars, table_vars)
                .unwrap(); // safe unwrap
            let lookup_vars = &[(0, 1, 1), (1, 1, 1)];
            let table_vars = &[(1, 1), (1, 1), (1, 1)];
            self.create_table_and_lookup_variables(lookup_vars, table_vars)
    ...
    /// Finalize the setup of the circuit before arithmetization for recursion.
    pub fn finalize_for_recursive_arithmetization<H: RecursionHasher>(
        &mut self,
    ) -> Result<(), CircuitError> {
        if self.is_finalized() {
            return Ok(());
    ...
    /// Finalize the setup of the circuit before MLE arithmetisation.
    pub fn finalize_for_mle_arithmetization(&mut self) -> Result<(), CircuitError> {
        if self.is_finalized() {
            return Ok(());
        }
        // here we add lookup variables to ensure no selectors are zero in the circuit
        ...
        let num_slots_needed =
            num_slots_needed
                .checked_next_power_of_two()
                .ok_or(CircuitError::InternalError(
        ...
        self.compute_extended_mle_id_permutation(num_slots_needed);
        Ok(())
    }

    /// Finalize the setup of the circuit before MLE arithmetisation.
    pub fn finalize_for_recursive_mle_arithmetization<H: RecursionHasher>(
        &mut self,
    ) -> Result<(), CircuitError> {
        if self.is_finalized() {
            return Ok(());
    ...
    F: PrimeField,
{
    ...
    fn num_mle_vars(&self) -> Result<usize, CircuitError> {
        ...
        let gate_count = self.num_gates() as u32;
        Ok(gate_count.ilog2() as usize)
    }

    fn compute_selector_mles(
        &self,
    ) -> Result<Vec<Arc<DenseMultilinearExtension<F>>>, CircuitError> {
        self.check_finalize_flag(true)?;
        // order: (lc, mul, hash, o, c, ecc) as specified in spec
        let num_vars = self.num_mle_vars()?;
        let selectors = self.all_selectors();
        let selector_mles = cfg_iter!(&selectors)
            .map(|selector| {
    ...
    fn compute_extended_permutation_mles(
        ...
        let num_vars = self.num_mle_vars()?;
        let extended_perm = self.compute_extended_mle_permutation(n)?;

        let extended_perm_polys: Vec<Arc<DenseMultilinearExtension<F>>> =
            cfg_into_iter!((0..self.num_wire_types())) // current par_utils only support slice iterator, not range iterator.
                .map(|i| {
                    Arc::new(DenseMultilinearExtension::from_evaluations_slice(
                        num_vars,
                        &extended_perm[i * n..(i + 1) * n],
                    ))
                })
                .collect();
    ...
    fn compute_extended_identity_mles(
        ...
        let extended_perm_polys: Vec<Arc<DenseMultilinearExtension<F>>> =
            cfg_into_iter!(self.extended_id_permutation()) // current par_utils only support slice iterator, not range iterator.
                .chunks(n)
                .map(|i| {
                    let evals = i.iter().map(|&f| *f).collect();
    ...
    fn compute_prod_permutation_mles(
        ...
        // If there are not the same number of wire and perm mles we should error.
        if wire_mles.len() != self.num_wire_types() || perm_mles.len() != self.num_wire_types() {
            return Err(ParameterError(
                "Number of wire and perm mles should match number of wire types".to_string(),
            ));
        }

        let wire_evals = wire_mles
            .iter()
            .map(|mle| {
                mle.to_evaluations()
                    .iter()
        ...
        let perm_evals = perm_mles
            .iter()
            .map(|mle| mle.to_evaluations())
            .collect::<Vec<_>>();

        // We take chunks of at most three wire and perm mles at a time and then the numerator is:
        // sigma_1 * (gamma - w_2) * (gamma - w_3) + (gamma - w_1) * sigma_2 * (gamma - w_3) + (gamma - w_1) * (gamma - w_2) * sigma_3
        // and the denominator is:
        // (gamma - w_1) * (gamma - w_2) * (gamma - w_3)

        let pairs = cfg_chunks!(perm_evals, 3)
            .zip(cfg_chunks!(wire_evals, 3))
            .map(|(perm_chunk, wire_chunk)| {
                let length = perm_chunk.len();
                let denominator: Vec<F> =
                    wire_chunk
                        .iter()
                        .fold(vec![F::one(); 1 << num_vars], |acc, wire_evals| {
                            acc.iter()
                                .zip(wire_evals.iter())
                                .map(|(acc, wire_eval)| *acc * *wire_eval)
                                .collect()
                        });
                let numerator = (0..length)
                    .map(|j| {
                        perm_chunk
                            .iter()
                            .enumerate()
                            ...
                            .fold(vec![F::one(); 1 << num_vars], |acc, evals| {
                                acc.iter()
                                    .zip(evals.iter())
                                    .map(|(acc, eval)| *acc * *eval)
                                    .collect()
                            })
                    })
                    .fold(vec![F::zero(); 1 << num_vars], |acc, evals| {
                        acc.iter()
                            .zip(evals.iter())
                            .map(|(acc, eval)| *acc + *eval)
                            .collect()
    ...
    fn compute_wire_mles(&self) -> Result<Vec<Arc<DenseMultilinearExtension<F>>>, CircuitError> {
        ...
        let witness = self.witness_vals();
        let num_vars = self.num_mle_vars()?;
        let wire_polys: Vec<Arc<DenseMultilinearExtension<F>>> = cfg_iter!(&self.wire_variables)
            .take(self.num_wire_types())
            .map(|wire_vars| {
                let wire_vec: Vec<F> = wire_vars.iter().map(|&var| witness[var]).collect();
                Arc::new(DenseMultilinearExtension::from_evaluations_slice(
                    num_vars, &wire_vec,
                ))
            })
    ...
    fn compute_pub_input_mle(&self) -> Result<Arc<DenseMultilinearExtension<F>>, CircuitError> {
        ...
        let num_vars = self.num_mle_vars()?;
        let witness = self.witness_vals();
        let wire_variables = &self.wire_variables;

        let mut pub_input_vec = vec![F::zero(); self.num_gates()];
        self.pub_input_gate_ids().iter().for_each(|&io_gate_id| {
            let var = wire_variables[GATE_WIDTH][io_gate_id];
            pub_input_vec[io_gate_id] = witness[var];
        });
    ...
    fn compute_range_table_mle(&self) -> Result<Arc<DenseMultilinearExtension<F>>, CircuitError> {
        ...
        let num_vars = self.num_mle_vars()?;
        let range_size = self.range_size()?;
        let size = self.num_gates();
        if size < range_size {
            return Err(ParameterError(format!(
                "Circuit size {} < range size {}",
                size, range_size
            )));
        }
        let mut range_table: Vec<F> = (0..range_size).map(|i| F::from(i as u32)).collect();
        range_table.resize(size, F::zero());
        Ok(Arc::new(DenseMultilinearExtension::from_evaluations_vec(
            num_vars,
            range_table,
        )))
    ...
    fn compute_merged_lookup_table_mle(&self, tau: F) -> Result<Vec<F>, CircuitError> {
        let range_size = self.range_size()?;
        let size = self.num_gates();
        if size < range_size {
            return Err(ParameterError(format!(
    ...
    fn compute_lookup_sorted_vec_mles(
        ...
        let num_vars = self.num_mle_vars()?;
        let n = self.num_gates();
        if n != self.wire_variables()[5].len() {
            return Err(ParameterError(
                "Domain size should match the size of the padded lookup variables vector"
                    .to_string(),
            ));
        }
        if n != lookup_table.len() {
            return Err(ParameterError(
                "Domain size should match the size of the padded lookup table".to_string(),
            ));
        }
        ...
        let q_lookup_vec = self.q_lookup();
        let q_dom_sep_vec = self.q_dom_sep();
        let mut merged_lookup_wire = vec![];
        for i in 0..n {
            merged_lookup_wire.push(self.merged_lookup_wire_value(
                tau,
                i,
                &q_lookup_vec,
                &q_dom_sep_vec,
...
impl<F: PrimeField> PlonkCircuit<F> {
    ...
    #[inline]
    pub fn compute_range_table(&self) -> Result<Vec<F>, CircuitError> {
        self.check_plonk_type(PlonkType::UltraPlonk)?;
        self.check_finalize_flag(true)?;
        let domain = &self.eval_domain;
        let range_size = self.range_size()?;
    ...
    /// Computes a merged lookup table value.
    #[inline]
    pub fn merged_table_value(
        &self,
        tau: F,
        range_table: &[F],
        table_key_vec: &[F],
    ...
    /// Computes a merged lookup table value for multivariate use.
    #[inline]
    #[allow(clippy::too_many_arguments)]
    pub fn merged_mv_table_value(
        &self,
        taus: &[F; 3],
        range_table: &[F],
        table_key_vec: &[F],
        ...
        let key_val = table_key_vec[i];
        let dom_sep_val = table_dom_sep_vec[i];
        let table_val_1 = wire_1[i];
        let table_val_2 = wire_2[i];
        Ok(range_val
            + dom_sep_val * taus[0]
            + taus[1] * key_val
            + taus[0] * taus[1] * table_val_1
            + taus[2] * table_val_2)
    }

    /// Computes a merged lookup wire value.
    #[inline]
    pub fn merged_lookup_wire_value(
        &self,
        tau: F,
        i: usize,
        q_lookup_vec: &[F],
    ...
    /// Computes a merged lookup wire value for multivariate use.
    #[inline]
    pub fn merged_mv_lookup_wire_value(
        &self,
        taus: &[F; 3],
        i: usize,
        lk_wire_one: &[F],
        ...
        let w_range_val = self.witness(self.wire_variable(RANGE_WIRE_ID, i))?;
        let lookup_key = lk_wire_one[i];
        let lookup_val_1 = lk_wire_two[i];
        let lookup_val_2 = lk_wire_three[i];
        let q_dom_sep_val = q_dom_sep_vec[i];
        Ok(w_range_val
            + q_dom_sep_val * taus[0]
            + taus[1] * lookup_key
            + taus[0] * taus[1] * lookup_val_1
            + taus[2] * lookup_val_2)
    }
}
...
pub(crate) mod test {
    ...
    use ark_ff::PrimeField;
    use ark_poly::{
        domain::Radix2EvaluationDomain, univariate::DensePolynomial, EvaluationDomain,
    ...
    fn test_circuit_trait_helper<F: PrimeField>() -> Result<(), CircuitError> {
        ...
        // 2 constant gates for default 0/1, 6 arithmetic gates
    ...
    #[test]
    fn test_lin_comb() -> Result<(), CircuitError> {
        test_lin_comb_helper::<FqEd254>()?;
        test_lin_comb_helper::<FqEd377>()?;
        test_lin_comb_helper::<FqEd381>()?;
        test_lin_comb_helper::<Fq377>()
    }

    fn test_lin_comb_helper<F: PrimeField>() -> Result<(), CircuitError> {
        let rng = &mut test_rng();
        for _ in 0..20 {
            let mut circuit: PlonkCircuit<F> = PlonkCircuit::new_turbo_plonk();
            let num_terms = (usize::rand(rng) % 15) + 2;
            let mut vec_coeff = Vec::<F>::new();
            let constant = F::rand(rng);
            let mut vec_var = Vec::<Variable>::new();
            let mut exp_val = constant;
            for _ in 0..num_terms {
                let coeff = F::rand(rng);
                vec_coeff.push(coeff);
                let val = F::rand(rng);
                let var = circuit.create_variable(val)?;
                vec_var.push(var);
                exp_val += coeff * val;
            }
            let res_var = circuit.lin_comb(&vec_coeff, &constant, &vec_var)?;
            assert_eq!(circuit.witness(res_var)?, exp_val);
    ...
    fn check_wire_permutation_and_extended_id_permutation<F: PrimeField>(
        ...
        circuit.pad(n)?;
        circuit.compute_wire_permutation(n);
    ...
    fn check_mle<F: PrimeField>(poly: &DenseMultilinearExtension<F>, evals: &[F]) {
        for (poly_eval, eval) in poly.iter().zip(evals.iter()) {
            assert_eq!(poly_eval, eval);
        }
    }

    #[test]
    fn test_mle_arithmetisation() {
        let (mut circuit, _pub_inputs) = create_ultra_plonk_instance::<Fq>().unwrap();
        circuit.finalize_for_mle_arithmetization().unwrap();

        test_arithmetization_for_mle_lookup_circuit(&circuit).unwrap();
    }

    pub(crate) fn test_arithmetization_for_mle_lookup_circuit<F: PrimeField>(
        circuit: &PlonkCircuit<F>,
    ) -> Result<(), CircuitError> {
        let n = circuit.num_gates();

        // Check range table polynomial
        let range_table_poly = circuit.compute_range_table_mle()?;
        let range_size = circuit.range_size()?;
        let mut range_table: Vec<F> = (0..range_size).map(|i| F::from(i as u32)).collect();
        range_table.resize(circuit.num_gates(), F::zero());
        check_mle(&range_table_poly, &range_table);

        // Check key table polynomial
        let key_table_poly = circuit.compute_key_table_mle()?;
        let key_table = circuit.table_key_vec();
        check_mle(&key_table_poly, &key_table);

        // Check sorted vector polynomials
        let rng = &mut test_rng();
        let tau = F::rand(rng);
        let merged_lookup_table = circuit.compute_merged_lookup_table_mle(tau)?;

        let gate_count = circuit.num_gates() as u32;
        let num_vars = gate_count.ilog2() as usize;

        let table = Arc::new(DenseMultilinearExtension::from_evaluations_vec(
            num_vars,
            merged_lookup_table.clone(),
        ));
        let lookup_wire = circuit.compute_lookup_sorted_vec_mles(tau, &merged_lookup_table)?;
        let wires = circuit.compute_wire_mles()?;
        let range_wire = circuit.compute_wire_mles()?.last().unwrap().clone();

        let q_dom_sep_vec = circuit.q_dom_sep();
        let q_lookup_vec = circuit.q_lookup();
        let mut merged_lookup_wire_vals = vec![];
        for i in 0..(n - 1) {
            let elem = circuit.merged_lookup_wire_value(tau, i, &q_lookup_vec, &q_dom_sep_vec)?;
            merged_lookup_wire_vals.push(elem);
        }
        merged_lookup_wire_vals.push(F::zero());

        let mut multiplicities = HashMap::new();
        let table_evals = table.to_evaluations();
        table_evals.iter().for_each(|&eval| {
            multiplicities
                .entry(eval)
        ...
        let mut lookup_multiplicities = HashMap::<F, F>::new();
        ...
        lookup_wire
            ...
            .iter()
            .map(|&eval| {
                if multiplicities.get(&eval).is_none() {
        ...
        let m_evals = table
            ...
            .map(|&value| {
                if let Some(lkup_m) = lookup_multiplicities.get(&value) {
                    ...
                    *lkup_m / multiplicities.get(&value).unwrap()
                } else {
                    F::zero()
                }
            })
        ...
        let m_poly = Arc::new(DenseMultilinearExtension::from_evaluations_vec(
            num_vars, m_evals,
        ));
        ...
        let table_key_mle = circuit.compute_key_table_mle()?;
        ...
        for i in 0usize..(1 << num_vars) {
            let point = bit_decompose::<F>(i as u64, num_vars);

            let m_eval = m_poly.evaluate(&point).unwrap();
            let a_m_w_1 = alpha - lookup_wire.evaluate(&point).unwrap();
            let a_m_w = alpha
                - (range_wire.evaluate(&point).unwrap()
                    + tau
                        * q_lookup_mle.evaluate(&point).unwrap()
                        * (q_dom_sep_mle.evaluate(&point).unwrap()
                            + tau
                                * (wires[0].evaluate(&point).unwrap()
                                    + tau
                                        * (wires[1].evaluate(&point).unwrap()
                                            + tau * wires[2].evaluate(&point).unwrap()))));
            let a_m_t = alpha
                - (range_table_mle.evaluate(&point).unwrap()
                    + tau
                        * q_lookup_mle.evaluate(&point).unwrap()
                        * (table_dom_sep_mle.evaluate(&point).unwrap()
                            + tau
                                * (table_key_mle.evaluate(&point).unwrap()
                                    + tau
                                        * (wires[3].evaluate(&point).unwrap()
                                            + tau * wires[4].evaluate(&point).unwrap()))));
            let a_m_t_1 = alpha - table.evaluate(&point).unwrap();
            assert_eq!(a_m_w_1, a_m_w);
            assert_eq!(a_m_t_1, a_m_t);
            sum += (m_eval / a_m_t) - (F::one() / a_m_w);
        }

        assert_eq!(F::zero(), sum);

        Ok(())
    }

    pub fn bit_decompose<F: PrimeField>(input: u64, num_var: usize) -> Vec<F> {
        let mut res = Vec::with_capacity(num_var);
        let mut i = input;
        for _ in 0..num_var {
            res.push(F::from(i & 1 == 1));
            i >>= 1;
        }
        res
    }
