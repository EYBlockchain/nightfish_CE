...
/// Does permutation stuff for Rescue
pub trait PermutationGadget<R, T, F>: Circuit<F>
where
    R: SpongeStateVar<T, F>,
    F: PrimeField,
{
    /// Checks bounds on a rescue state
    fn check_var_bound_rescue_state(&self, rescue_state: &R) -> Result<(), CircuitError>;

    /// Adds constants to a rescue vector
