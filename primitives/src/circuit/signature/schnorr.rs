...
use ark_ec::{
    short_weierstrass::Affine as SWAffine, twisted_edwards::TECurveConfig as Config, AffineRepr,
    CurveGroup,
};
...
use jf_relation::{
    errors::CircuitError,
    gadgets::ecc::{HasTEForm, Point, PointVariable},
    ...
    P: Config<BaseField = F> + HasTEForm<BaseField = F>,
{
    ...
    fn create_signature_variable(
        ...
        let sig_var = SignatureVar {
            ...
            R: self.create_point_variable(&Point::from(sig.R.into_affine()))?,
        };
        Ok(sig_var)
    }

    fn create_signature_vk_variable(&mut self, vk: &VerKey<P>) -> Result<VerKeyVar, CircuitError> {
        let vk_var = VerKeyVar(self.create_point_variable(&Point::from(vk.0.into_affine()))?);
        Ok(vk_var)
    }
