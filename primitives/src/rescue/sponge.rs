...
use jf_relation::{errors::CircuitError, RecursionHasher};
use jf_utils::{bytes_to_field_elements, pad_with_zeros};
...
impl<F: RescueParameter> RecursionHasher for RescueCRHF<F> {
    ...
    fn hash_public_inputs<E: PrimeField>(public_inputs: &[E]) -> Result<E, Self::Error> {
        ...
        for msg in public_inputs.iter() {
            if size_check {
                let msg = F::from_le_bytes_mod_order(&msg.into_bigint().to_bytes_le());
                input.push(msg);
            } else {
                let mut writer = Vec::new();
                msg.serialize_compressed(&mut writer).map_err(|_| {
                    CircuitError::InternalError("Could not serialize field element".to_string())
                })?;
