...
impl<F: PrimeField> RescueVector<F> {
    ...
    /// Creates a new vector from the field elements.
    pub fn new(state: [F; STATE_SIZE]) -> RescueVector<F> {
        RescueVector { vec: state }
    }
}
...
impl<F: PrimeField> RescueVector<F> {
    ...
    fn add_assign(&mut self, vector: &RescueVector<F>) {
        self.vec = [
            self.vec[0] + vector.vec[0],
            self.vec[1] + vector.vec[1],
            self.vec[2] + vector.vec[2],
            self.vec[3] + vector.vec[3],
        ];
    }
...
impl<F: PrimeField> RescueMatrix<F> {
    fn mul_vec(&self, vector: &RescueVector<F>) -> RescueVector<F> {
        RescueVector {
            vec: [
                self.matrix[0].dot_product(vector),
                self.matrix[1].dot_product(vector),
                self.matrix[2].dot_product(vector),
                self.matrix[3].dot_product(vector),
            ],
        }
    }
...
mod test_prp {
    ...
    // this value is cross checked with sage script
    // rescueGrumpkin.Sponge([0,0,0,0], 4)
    const OUTPUTGRUMPKIN: [[u8; 32]; 4] = [
        [
    ...
    fn test_rescue_perm_on_0_vec() {
        ...
        test_rescue_perm_on_0_vec_grumpkin();
    }

    fn test_rescue_perm_on_0_vec_grumpkin() {
        let rescue = PRP::<Fq254>::default();
        let key = RescueVector::<Fq254>::zero();
        let input = RescueVector::<Fq254>::zero();
...
mod test_permutation {
    ...
    // hash output on vector [0, 0, 0, 0]
    // this value is cross checked with sage script
    // first three vectors of rescueGrumpkin.Sponge([0,0,0,0], 4)

    const OUTPUTGRUMPKIN: [[u8; 32]; 3] = [
        [
            0xC9, 0xAC, 0x42, 0x08, 0x04, 0x18, 0xFC, 0x10, 0xA4, 0x41, 0xF4, 0xF9, 0x06, 0x2B,
            0xE2, 0x87, 0xB7, 0x7A, 0xC4, 0x65, 0x8C, 0xE0, 0xE9, 0x78, 0xB0, 0x48, 0x39, 0xB6,
            0x96, 0x9B, 0x60, 0x1B,
    ...
    fn test_rescue_hash_on_0_vec() {
        ...
        test_rescue_hash_on_0_vec_grumpkin();
        test_rescue_hash_on_0_vec_761()
    }

    fn test_rescue_hash_on_0_vec_grumpkin() {
        let input = [Fq254::zero(); 3];
        let expected = vec![
            Fq254::from_le_bytes_mod_order(&OUTPUTGRUMPKIN[0]),
            Fq254::from_le_bytes_mod_order(&OUTPUTGRUMPKIN[1]),
            Fq254::from_le_bytes_mod_order(&OUTPUTGRUMPKIN[2]),
        ];
