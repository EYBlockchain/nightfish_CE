...
use crate::pcs::{prelude::Commitment, PCSError, StructuredReferenceString};
...
pub struct UnivariateProverParam<E: Pairing> {
    /// Config
    pub powers_of_g: Vec<Commitment<E>>,
}

impl<E: Pairing> Default for UnivariateProverParam<E> {
    fn default() -> Self {
        Self {
            powers_of_g: Vec::<Commitment<E>>::new(),
        }
    }
}
...
/// commitment.
#[derive(Derivative, CanonicalSerialize, CanonicalDeserialize, Clone, Copy)]
#[derivative(
    Default(bound = ""),
    Debug(bound = ""),
    PartialEq(bound = ""),
