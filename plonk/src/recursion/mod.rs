...
pub trait RecursiveProver {
        ...
        let bn254_outputs: [Bn254Output; 2] = circuits
            ...
            .map(|circuit| {
                let fs_msg = {
                    ...
                    let layer = if fs_metadata.recursion_depth == 2 {
                        "base_bn254"
                    } else {
                        "merge_bn254"
                    };
                ...
                FFTPlonk::<Kzg>::recursive_prove(&mut rng, &circuit, bn254_pk, Some(fs_msg), false)
            })
    ...
    fn prove(
        ...
        ark_std::println!("vk = {:?}", hash_canonical(&decider_pk.vk));
        ark_std::println!("rollup size {}", rollup_size);
        ark_std::println!("recursion depth {}", recursion_depth);
        ark_std::println!("srs_digest {:?}", srs_digest);
        ark_std::println!("fs_msg {:?}", fs_msg);
...
mod tests {
    ...
    fn test_preprocess_and_prove() -> Result<(), PlonkError> {
        ...
        let binding = Sha256::digest(
            kzg_srs
                ...
                .flat_map(|p| {
                    ...
                    p.serialize_compressed(&mut buffer).unwrap();
                    buffer
                })
                .collect::<Vec<u8>>(),
        );
        let srs_digest = &binding.into();
        ...
        let decider_depth = prove_inputs.len().ilog2() - 1;
        let fs_msg = fs_domain_bytes(
            "nightfish.pcd",
            "plonk-recursion",
            "v1",
