...
impl SplitAccumulationInfo {
    ...
    pub fn verify_split_accumulation(
        &self,
        grumpkin_proofs: &[SAMLEProofVar<Zmorph>; 2],
        accumulators: &[PCSWitness<Zmorph>; 4],
        ...
        let accumulation_coeffs = {
            let initial_var = circuit.create_emulated_variable(Fq254::one())?;
            iter::successors(Some(initial_var), |x| {
                circuit.emulated_mul_constant(x, self.batch_challenge).ok()
            })
            .take(6)
...
fn compute_mle_scalars(
    ...
    let scalars = accumulation_scalars
        ...
        .map(|(acc_scalar, delta)| {
            (0..35).try_fold(Vec::with_capacity(35), |mut result, _| {
                let current = if result.is_empty() {
                    acc_scalar.clone()
                } else {
