...
use crate::recursion::circuits::challenges::MLEProofChallengesVar;
...
pub fn prove_grumpkin_accumulation<const IS_BASE: bool>(
    ...
    let next_grumpkin_challenges: Vec<(
        MLEProofChallengesEmulatedVar<Fq254>,
            ...
            let grumpkin_accs = if !IS_BASE {
                grumpkin_accumulators
                    ...
                    .map(|(comm, eval)| {
                        ...
                        let hi_shifted =
                            circuit.emulated_mul_constant::<Fq254>(&high_e, radix_fq)?;
...
impl MLEProofChallengesVar {
    ...
    pub fn convert_to_hash_form(
        ...
        for var in out {
            let [low_var, high_var] = convert_to_hash_form_fq254(circuit, var).map_err(|_| {
                CircuitError::ParameterError("Couldn't convert to fixed length array".to_string())
            })?;
...
impl MLEProofChallengesEmulatedVar<Fq254> {
    ...
    pub fn convert_to_hash_form(
        ...
        let mut leftover_vec = Vec::<Variable>::new();
        for emu_var in out {
            let [low_var, high_var] = circuit
                .convert_for_transcript(&emu_var)?
                .try_into()
                .map_err(|_| {
                    CircuitError::ParameterError(
