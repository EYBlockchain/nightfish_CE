...
impl<PCS: PolynomialCommitmentScheme> MLEPlonk<PCS> {
        ...
        T: Transcript,
    {
        ...
        let () = check_proof_shape::<F, PCS, P>(&shared, vk, public_input, num_vars)?;
    ...
    {
        let mut transcript = T::new_transcript(b"mle_plonk");
        let proof: &SAMLEProof<PCS> = &recursion_output.proof;
        let num_vars = vk.num_vars as usize;
        let n = 1usize << num_vars;

        let shared = MLEProofShared::from(proof);
        let () = check_proof_shape(&shared, vk, &[public_input], num_vars)?;
