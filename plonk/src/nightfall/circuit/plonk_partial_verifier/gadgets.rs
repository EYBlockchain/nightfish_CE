...
pub fn compute_scalars_for_native_field<F: PrimeField + RescueParameter, const IS_BASE: bool>(
    ...
    let (domain_size_var, gen_var, gen_inv_var) = {
        ...
        #[cfg(not(test))]
        {
            if IS_BASE {
                ...
                let transfer_domain =
                    Radix2EvaluationDomain::<F>::new(TRANSFER_DOMAIN_SIZE).unwrap();
                ...
                let transfer_gen_var =
                    circuit.create_constant_variable(transfer_domain.group_gen)?;
                ...
                let gen_var = circuit.conditional_select(
                    is_transfer_var,
                    deposit_gen_var,
                    transfer_gen_var,
                )?;
