...
impl<E: Pairing> Prover<E> {
    ...
    fn compute_quotient_circuit_contribution(
        ...
        q_c + pi
            ...
            + q_hash[2] * w[2].pow([5])
            + q_hash[3] * w[3].pow([5])
            + q_x * (w[0] * w[3] * w[2] * w[3] + w[1] * w[2] * w[2] * w[3])
            + q_x2
                * (w[0] * w[2]
                    + w[1] * w[3]
                    + E::ScalarField::from(2u8) * w[0] * w[3]
                    + E::ScalarField::from(2u8) * w[1] * w[2])
            + q_y * (w[2] * w[2] * w[3] * w[3])
            + q_y2 * (w[0] * w[0] * w[1] + w[0] * w[1] * w[1])
            - q_o * w[4]
    }
        ...
        let mut split_quot_polys: Vec<DensePolynomial<E::ScalarField>> =
            parallelizable_slice_iter(&(0..num_wire_types).collect::<Vec<_>>())
                .map(|&i| {
                    ...
                    DensePolynomial::<E::ScalarField>::from_coefficients_slice(&coeff_chunk)
                })
                .collect();
